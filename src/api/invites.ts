...
type JoinResult =
  ...
  | {
      ok: false;
      reason: 'invalid' | 'expired' | 'already_member' | 'unknown';
      message?: string;
    };
...
export async function createInvite(campaignId: string): Promise<CampaignInvite> {
  ...
  if (Array.isArray(data)) {
    return data[0] as CampaignInvite;
  }
  return data as CampaignInvite;
}
...
export async function joinCampaignWithInviteCode(rawCode: string): Promise<JoinResult> {
  ...
  // Type/table shows: id, campaign_id, code, created_by, created_at
  ...
  if (!invite) {
    return { ok: false, reason: 'invalid', message: 'That invite code is not valid.' };
  }
